...
def emit_task_signal(sig1,  inc):
        ...
        global datamgr_proxy,  task_signal,   taskinfo
        print datamgr_proxy.mTaskInfo.items()
...
def emitter_main(datamgr,  dbus_iface= DBUS_IFACE,  dbus_path = DBUS_PATH, \
        ...
        global task_signal,  taskinfo,  datamgr_proxy
        ...
        print "items: %s" %datamgr.mTaskInfo.items()
        time.sleep(5)
        taskinfo = datamgr_proxy.mTaskInfo
        for v in taskinfo.items():
            print v
        try:
